...
const creatRadioBtns = (wrapper, name, data)=>{
    wrapper.textContent = '';
...
const createFormDay = (date)=>{
    ...
    creatRadioBtns(dayRadioWrapper,'day',typeData)
}
...
const createFormTime = (date,day) =>{
    ...
    creatRadioBtns(timeRadioWrapper,'time',daysData)
...
const renderTypeFieldset = () =>{
    ...
    creatRadioBtns(typeRadioWrapper, 'type', typeData)
}
...
form.addEventListener('input', handleInputForm)
    ...
    form.addEventListener('submit', async (e)=>{
        ...
        try{
            const response = await fetch  ('https://steel-cosmic-feels.glitch.me/api/orders', {
                method: 'Post',
                headers: {
            ...
            console.error(`Оибка при отправке запроса: ${error}`)
        }
    })
 }
